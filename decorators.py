    ...
    def decorator(fn):
        ...
        def check(*args, **kwargs):
            if user["access_level"] == access_level:
                return fn(*args, **kwargs)
            return "You do not have the access privilege"
        return check
    return decorator

@make_secure("user") # return decorator
def get_password_for_user(time):
    return "Password for user at time stamp " + time

@make_secure("admin")
def get_password_for_admin(time):
    return "Password for admin at time stamp " + time

print("When i am user:  ")
user  = {"user_name":"Nick", "access_level": "user"}
print(get_password_for_user("20200229"))
print(get_password_for_admin("20200229"))

print("When i am admin:  ")
user  = {"user_name":"Nick", "access_level": "user"}
print(get_password_for_user("20200229"))
print(get_password_for_admin("20200229"))

# permanent_residence_visa_applicant
from functools import wraps
pr_visa_applicant1 = {"name":"Vanissa", "race":"black","gender":"F"}
pr_visa_applicant2 = {"name":"Trump", "race":"white", "gender":"M"}
pr_visa_applicant3 = {"name":"Gong", "race":"yellow", "gender":"F"}
def grandcheck(gender):
    def decorator(fn):
        @wraps(fn)
        def check(*args, **kwargs):
            if kwargs.get("gender") == gender:
                return fn(*args, **kwargs)
            return "Not qualified for Permanent Residence application!"
        return check
    return decorator

@grandcheck("F")
def get_pr_decision(**applicant):
    if applicant["race"] == "black" or applicant["race"] == "white":
        return "sorry your permanent residence application for PRC has been rejected."
    return "Congratulations, your application has been granted."

print(get_pr_decision(**pr_visa_applicant1))
print(get_pr_decision(**pr_visa_applicant2))
print(get_pr_decision(**pr_visa_applicant3))
